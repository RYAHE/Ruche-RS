  ...
  <div class="category-page">
    ...
    <div class="posts-container">
      ...
      <div v-else-if="posts.length === 0" class="no-posts">
        <i class="fas fa-exclamation-triangle"></i>
        <p>Aucun post NSFW pour le moment</p>
...
export default {
  ...
  head() {
    return {
      title: 'NSFW - Ruche',
      meta: [
        { hid: 'description', name: 'description', content: 'Contenu pour adultes dans la communauté Ruche' }
      ]
    }
  },
    ...
    async loadCategoryId() {
      try {
        ...
        if (response.data.categories) {
          const nsfwCategory = response.data.categories.find(cat => 
            cat.nom.toLowerCase() === 'nsfw'
          )
          
          if (nsfwCategory) {
            this.categoryId = nsfwCategory.id
            this.newPost.categorieId = nsfwCategory.id
          } else {
            console.error('Catégorie "NSFW" non trouvée')
            this.$toast.error('Catégorie "NSFW" non trouvée')
            this.$router.push('/')
          }
        }
      } catch (error) {
