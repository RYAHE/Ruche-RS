<template>
  <div class="category-page">
    ...
    <!-- Bouton flottant pour poser une nouvelle question -->
    ...
    <!-- Modal pour créer une nouvelle question -->
      ...
      <div class="modal-content">
        ...
        <div class="post-editor">
          <div class="form-group">
            <label for="postTitle">Votre question</label>
            <input type="text" id="postTitle" v-model="newPost.titre" placeholder="Ex: Comment faire pour..., Où trouver..."
              maxlength="255" />
          </div>
          ...
          <textarea v-model="newPost.contenu" placeholder="Décrivez votre problème ou votre question en détail. Plus vous donnerez d'informations, plus les autres pourront vous aider efficacement..."
            :maxlength="maxContentLength"></textarea>

          <div class="question-options">
            <div class="anonymous-option">
              <input type="checkbox" id="anonymousPost" v-model="newPost.estAnonyme" />
              <label for="anonymousPost">Poser anonymement</label>
            </div>
            
            <div class="question-tip">
              <i class="fas fa-users"></i>
              <span>🤝 N'hésitez pas à donner du contexte pour obtenir les meilleures réponses</span>
            </div>
          </div>
        </div>
    ...
    <div v-if="showEditModal" class="modal" id="editPostModal">
      <div class="modal-content">
        ...
        <div class="post-editor">
          <div class="form-group">
            <label for="editPostTitle">Votre question</label>
            <input type="text" id="editPostTitle" v-model="editingPost.titre" placeholder="Votre question"
              maxlength="255" />
          </div>
          ...
          <textarea v-model="editingPost.contenu" placeholder="Décrivez votre problème ou votre question en détail..."
            :maxlength="maxContentLength"></textarea>

          <div class="anonymous-option">
            <input type="checkbox" id="editAnonymousPost" v-model="editingPost.estAnonyme" />
            <label for="editAnonymousPost">Poser anonymement</label>
          </div>
        </div>
...
export default {
  ...
  head() {
    return {
      title: 'Questions - Ruche',
      meta: [
        { hid: 'description', name: 'description', content: 'Posez vos questions et obtenez de l\'aide de la communauté Ruche' }
      ]
    }
  },
  ...
  methods: {
    async loadCategoryId() {
      try {
        ...
        if (response.data.categories) {
          const questionsCategory = response.data.categories.find(cat => 
            cat.nom.toLowerCase() === 'questions' || cat.nom.toLowerCase() === 'question'
          )
          
          if (questionsCategory) {
            this.categoryId = questionsCategory.id
            this.newPost.categorieId = questionsCategory.id
          } else {
            console.error('Catégorie "Questions" non trouvée')
            this.$toast.error('Catégorie "Questions" non trouvée')
            this.$router.push('/')
          }
        }
      } catch (error) {
    ...
    async loadPosts() {
      ...
      try {
        ...
        this.$toast.error('Erreur lors du chargement des questions')
        this.posts = []
      } finally {
        this.loading = false
      }
    ...
    async createPost() {
      ...
      try {
        ...
        this.$toast.success('Question posée avec succès!')
        this.showNewPostModal = false
        this.resetNewPost()
        await this.loadPosts()
      } catch (error) {
        console.error('Erreur lors de la création de la question:', error)
        this.$toast.error('Erreur lors de la création de la question')
      }
    },
    ...
    async updatePost() {
      ...
      try {
        ...
        this.$toast.success('Question modifiée avec succès!')
        this.showEditModal = false
        await this.loadPosts()
      } catch (error) {
        console.error('Erreur lors de la modification de la question:', error)
        this.$toast.error('Erreur lors de la modification de la question')
      }
    },
    ...
    async deletePost() {
      ...
      try {
        ...
        this.$toast.error('Erreur lors de la suppression de la question')
      }
    },
