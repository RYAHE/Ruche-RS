  ...
  <div class="category-page">
      ...
      <div class="modal-content">
        ...
        <div class="post-editor">
          <div class="form-group">
            <label for="postTitle">Titre de l'annonce</label>
            <input type="text" id="postTitle" v-model="newPost.titre" placeholder="Ex: Vends vélo, Recherche covoiturage..."
              maxlength="255" />
          </div>
          ...
          <textarea v-model="newPost.contenu" placeholder="Décrivez votre annonce en détail : prix, état, contact..."
          ...
          <div class="annonce-options">
            ...
            <div class="annonce-tip">
              <i class="fas fa-lightbulb"></i>
              <span>💡 Pensez à inclure vos informations de contact dans le contenu</span>
            </div>
          </div>
        </div>
    ...
    <div v-if="showEditModal" class="modal" id="editPostModal">
      <div class="modal-content">
        ...
        <div class="post-editor">
          <div class="form-group">
            <label for="editPostTitle">Titre de l'annonce</label>
            <input type="text" id="editPostTitle" v-model="editingPost.titre" placeholder="Titre de votre annonce"
              maxlength="255" />
          </div>
          ...
          <textarea v-model="editingPost.contenu" placeholder="Décrivez votre annonce en détail..."
            :maxlength="maxContentLength"></textarea>
...
export default {
  ...
  head() {
    return {
      title: 'ANNONCES - Ruche',
      meta: [
        { hid: 'description', name: 'description', content: 'Petites annonces, ventes, achats et échanges entre membres de la communauté Ruche' }
      ]
    }
  },
  ...
  methods: {
    async loadCategoryId() {
      try {
        ...
        if (response.data.categories) {
          const annoncesCategory = response.data.categories.find(cat => 
            cat.nom.toLowerCase() === 'annonces' || cat.nom.toLowerCase() === 'annonce'
          )
          
          if (annoncesCategory) {
            this.categoryId = annoncesCategory.id
            this.newPost.categorieId = annoncesCategory.id
          } else {
            console.error('Catégorie "Annonces" non trouvée')
            this.$toast.error('Catégorie "Annonces" non trouvée')
            this.$router.push('/')
          }
        }
      } catch (error) {
    ...
    async loadPosts() {
      ...
      try {
        ...
        this.$toast.error('Erreur lors du chargement des annonces')
        this.posts = []
      } finally {
        this.loading = false
      }
    ...
    async deletePost() {
      ...
      try {
        ...
        this.$toast.error('Erreur lors de la suppression de l\'annonce')
      }
    },
