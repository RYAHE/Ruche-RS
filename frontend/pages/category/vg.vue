  ...
  <div class="category-page">
    ...
    <div class="posts-container">
      ...
      <div v-else-if="posts.length === 0" class="no-posts">
        <i class="fas fa-gamepad"></i>
        <p>Aucun post VG pour le moment</p>
...
export default {
  ...
  head() {
    return {
      title: 'VG - Ruche',
      meta: [
        { hid: 'description', name: 'description', content: 'Discussions sur les jeux vidéo dans la communauté Ruche' }
      ]
    }
  },
    ...
    async loadCategoryId() {
      try {
        ...
        if (response.data.categories) {
          const vgCategory = response.data.categories.find(cat => 
            cat.nom.toLowerCase() === 'vg' || cat.nom.toLowerCase() === 'jeux vidéo'
          )
          
          if (vgCategory) {
            this.categoryId = vgCategory.id
            this.newPost.categorieId = vgCategory.id
          } else {
            console.error('Catégorie "VG" non trouvée')
            this.$toast.error('Catégorie "VG" non trouvée')
            this.$router.push('/')
          }
        }
      } catch (error) {
