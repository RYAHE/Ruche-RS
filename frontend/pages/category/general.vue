  ...
  <div class="category-page">
    ...
    <div class="posts-container">
      ...
      <div v-else-if="posts.length === 0" class="no-posts">
        <i class="fas fa-random"></i>
        <p>Aucun post RANDOM pour le moment</p>
    ...
    <!-- Bouton flottant pour créer un nouveau post -->
...
export default {
  ...
  head() {
    return {
      title: 'RANDOM - Ruche',
      meta: [
        { hid: 'description', name: 'description', content: 'Discussions générales et échanges libres entre membres de la communauté Ruche' }
      ]
    }
  },
    ...
    async loadCategoryId() {
      try {
        ...
        if (response.data.categories) {
          const generalCategory = response.data.categories.find(cat => 
            cat.nom.toLowerCase() === 'général' || cat.nom.toLowerCase() === 'general' || cat.nom.toLowerCase() === 'random'
          )
          
          if (generalCategory) {
            this.categoryId = generalCategory.id
            this.newPost.categorieId = generalCategory.id
          } else {
            console.error('Catégorie "RANDOM/Général" non trouvée')
            this.$toast.error('Catégorie "RANDOM" non trouvée')
            this.$router.push('/')
          }
        }
      } catch (error) {
